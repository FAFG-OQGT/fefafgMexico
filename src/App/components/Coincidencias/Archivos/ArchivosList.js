...
function ArchivosList({CoincidenciaId, reset, setreset, mensajeAlerta}) {
  ...
  const fetchFileDownload = async (urlPath, fileName) => {
    try {
      const res = await axios.get(
        `${config.urlApi}/coincidencia/archivo/?key=${urlPath}`,
  ...
  const fetchFileDelete = async (urlPath, coincidencia) => {
    try {
      ...
      const res = await axios.post(
        `${config.urlApi}/coincidencia/deleteArchivo`,
        data,
        configReq
      );
      if (res.status === 201) {
  ...
  const fetchArchivos = async () => {
    try {
      const res = await axios.get(
        `${config.urlApi}/coincidencia/documento/${CoincidenciaId}`,
      ...
      list.forEach((elementList, index) => {
        if (elementList.padrePath === element.padrePath)
          childrens.push({
            ...
            isLeaf: true,
            coincidenciaDocumentoId: elementList.coincidenciaDocumentoId,
      ...
      if (data !== undefined) {
        if (data.length > 0) {
          data.map(function (d) {
            ...
            treeD.push({
              id: contador,
              coincidenciaDocumentoId: d.coincidenciaDocumentoId,
