...
function AnotacionesDidList({CoincidenciaId}) {
  ...
  const fetchAnotaciones = async () => {
    try {
      const res = await axios.get(
        `${config.urlApi}/coincidencia/anotacion/${CoincidenciaId}`,
    ...
    <div>
      <Card className="table-card review-card">
        <Card.Body>
          {data.length > 0 ? (
            data.map((fbb) => (
              <div key={fbb.anotacionCoincidenciaId} className="review-block">
                <div className="row">
                  <div className="col">
...
export default AnotacionesDidList;
