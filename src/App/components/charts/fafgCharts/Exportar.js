...
import iconoEmpresa from "../../../../assets/images/CRIHLOGO2.png";
...
export const Exportar = ({
  id,
  nombre = "descarga",
  subTitulo = "Dirección de generación de reportes",
  descripcion = "Reporte de Graficas",
}) => {
  const addHeadFood = (pdf, totalPaginas = 1, pagina = 1) => {
    ...
    pdf.setFontSize(12);
    const titulo =
      "Centro Regional de Identificación Humana del Estado de Coahuila de Zaragoza";
    const calculoXTitulo =
      pdf.internal.pageSize.width / 2 -
      (pdf.getStringUnitWidth(titulo) * pdf.internal.getFontSize()) / 2;
    pdf.text(titulo, calculoXTitulo, 40);

    pdf.setFontSize(10);
    const calculoXSubTitulo =
      pdf.internal.pageSize.width / 2 -
      (pdf.getStringUnitWidth(subTitulo) * pdf.internal.getFontSize()) / 2;
    pdf.text(subTitulo, calculoXSubTitulo, 52);

    pdf.setFontSize(12);
    const calculoXDescripcionReporte =
      pdf.internal.pageSize.width / 2 -
      (pdf.getStringUnitWidth(descripcion) * pdf.internal.getFontSize()) / 2;
    ...
    const calculoDescripcionPaginas =
      pdf.internal.pageSize.width -
      20 -
      pdf.getStringUnitWidth(descripcionPaginas) * pdf.internal.getFontSize();
      ...
      <Col className="d-flex justify-content-end">
        {id !== undefined ? (
          <>
            ...
            <button className="btn btn-info btn-sm mr-1" onClick={handleJpg}>
              Image
            </button>
          </>
        ) : (
          <button
            className="btn-icon btn btn-outline-primary btn-sm"
            onClick={handleTodos}
          >
