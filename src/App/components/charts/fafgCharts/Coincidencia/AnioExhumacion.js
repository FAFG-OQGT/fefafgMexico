...
export const AnioExhumacion = ({ titulo,data,id="graficaAnioExhumacion",optionsBar,optionsPie }) => {
    const [tipoChart, settipoChart] = useState(true);
    const getData =(canvas) => {
        if(tipoChart){
            return GetDataCoincidencia(types.osamentaAnioExhumacion, data, types.chartBar,canvas);
        }else{
            return GetDataCoincidencia(types.osamentaAnioExhumacion, data, types.chartPie,canvas);
        }
    }
           ...
           <Card className={"h-100"}>
                ...
                <Card.Body>
                    ...
                    <Col>
                        {
                            tipoChart ?
                            <BarBasicChart id={id} data={getData} options={optionsBar} />
                            : <PieChart id={id} data={getData} options={optionsPie} />
                        }
                        </Col>
                    </Row>
                </Card.Body>
