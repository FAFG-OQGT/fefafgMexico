...
function ArchivosAdd({ IdentificadoId, mensajeAlerta, onAddDoneFile }) {
  ...
  const handleAdd = (e) => {
    ...
    if (
      ...
      if (file.size > config.maxSizeOst) {
        Swal.fire(
          "Alerta",
      ...
      nombreNuevo = nombreNuevo
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "");
      //data.append("File", file,nombreNuevo+'_IO'+IdentificadoId.toString() + "." + ext);
      data.append("File", file, file.name);
      data.append("identificadoOstId", IdentificadoId);
  ...
  const createFileIdentificado = async (data) => {
    try {
      ...
      const res = await axios.post(
        `${config.urlApi}/IdentificadoOst/archivo`,
        data,
        configReq
      );
      if (res.status === 202) {
  ...
  const fetchCatalogo = async (catalogo) => {
    ...
    if (catalogo == "documentoIdentificadoOst") setcombodocumento(result.data);
  ...
  useEffect(() => {
    ...
    fetchCatalogo("documentoIdentificadoOst");
