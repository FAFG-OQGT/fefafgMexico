...
export const BasedeDatos = ({ titulo,data,id="graficaBasedeDatos",optionsBar,optionsPie }) => {
    const [tipoChart, settipoChart] = useState(true);
    const getData =(canvas) => {
        if(tipoChart){
            return GetDataCoincidencia(types.baseInfo, data, types.chartBar,canvas);
        }else{
            return GetDataCoincidencia(types.baseInfo, data, types.chartPie,canvas);
        }
    }
