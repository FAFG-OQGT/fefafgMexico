...
function NotasLaboratorioList({CoincidenciaId}) {
  ...
  const fetchNotas = async () => {
    try {
      const res = await axios.get(
        `${config.urlApi}/coincidencia/notaLab/${CoincidenciaId}`,
    ...
    <div>
      <Card className="table-card review-card">
        <Card.Body>
          {data.length > 0 ? (
            data.map((fbb) => (
              <div key={fbb.notaLabGeneticaId} className="review-block">
                <div className="row">
                  <div className="col">
...
export default NotasLaboratorioList;
