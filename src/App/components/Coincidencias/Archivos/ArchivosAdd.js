...
function ArchivosAdd({ CoincidenciaId, mensajeAlerta, onAddDoneFile }) {
  ...
  const handleAdd = (e) => {
    ...
    if (
      ...
      if (file.size > config.maxSizeCoincidencia) {
        Swal.fire(
          "Alerta",
      ...
      createFileCoincidencia(data);
    }
  };

  const createFileCoincidencia = async (data) => {
    try {
      setloading(true);
      const res = await axios.post(
        `${config.urlApi}/coincidencia/archivo`,
        data,
        configReq
      );
      if (res.status === 202) {
  ...
  const fetchCatalogo = async (catalogo) => {
    ...
    if (catalogo == "documentoCoincidencia") setcombodocumento(result.data);
  ...
  useEffect(() => {
    ...
    fetchCatalogo("documentoCoincidencia");
